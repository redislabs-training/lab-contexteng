# MATH022: Linear Algebra for Machine Learning

## Course Information

- **Department**: Mathematics
- **Credits**: 4
- **Difficulty Level**: intermediate
- **Format**: in_person
- **Instructor**: Patty Perez
- **Semester**: winter 2025
- **Max Enrollment**: 69

## Description

Vectors, matrices, and linear transformations for ML applications. This course provides the mathematical foundation essential for understanding machine learning algorithms. Topics include vector spaces, eigenvalues, SVD, and their applications in dimensionality reduction, recommendation systems, and neural networks.

## Learning Objectives

- Understand core concepts in linear algebra for machine learning
- Implement linear algebra for machine learning algorithms and techniques
- Apply linear algebra for machine learning to real-world problems
- Analyze and evaluate linear algebra for machine learning solutions
- Design and build complete mathematics systems

## Assignments

**Total Points**: 1250


### Exams

#### Midterm Exam (Week 7)

Comprehensive exam covering first half of course

- **Points**: 200
- **Estimated Hours**: 3.0


### Homeworks

#### Homework 1 (Week 2)

Problem set covering weeks 1-2

- **Points**: 100
- **Estimated Hours**: 8.0
- **Submission Format**: PDF or Jupyter Notebook

#### Homework 2 (Week 4)

Problem set covering weeks 3-4

- **Points**: 100
- **Estimated Hours**: 8.0
- **Submission Format**: PDF or Jupyter Notebook

#### Homework 3 (Week 6)

Problem set covering weeks 5-6

- **Points**: 100
- **Estimated Hours**: 8.0
- **Submission Format**: PDF or Jupyter Notebook

#### Homework 4 (Week 8)

Problem set covering weeks 7-8

- **Points**: 100
- **Estimated Hours**: 8.0
- **Submission Format**: PDF or Jupyter Notebook

#### Homework 5 (Week 10)

Problem set covering weeks 9-10

- **Points**: 100
- **Estimated Hours**: 8.0
- **Submission Format**: PDF or Jupyter Notebook

#### Homework 6 (Week 12)

Problem set covering weeks 11-12

- **Points**: 100
- **Estimated Hours**: 8.0
- **Submission Format**: PDF or Jupyter Notebook


### Projects

#### Project Proposal (Week 10)

1-page proposal for final project

- **Points**: 50
- **Estimated Hours**: 4.0
- **Group Work**: Yes
- **Submission Format**: PDF

#### Project Draft (Week 12)

Working implementation and preliminary results

- **Points**: 100
- **Estimated Hours**: 15.0
- **Group Work**: Yes
- **Submission Format**: GitHub Repository

#### Final Project (Week 14)

Complete project with code, report, and presentation

- **Points**: 300
- **Estimated Hours**: 25.0
- **Group Work**: Yes
- **Submission Format**: GitHub + Presentation


## Course Syllabus (14 weeks)


### Week 1: Vectors and Vector Spaces

**Subtopics**:
- Vectors and Vector Spaces - Part 1
- Vectors and Vector Spaces - Part 2

**Learning Objectives**:
- Understand vectors and vector spaces
- Apply vectors and vector spaces concepts
- Implement vectors and vector spaces solutions

**Readings**:
- Chapter 1
- Research Paper 1


### Week 2: Matrix Operations

**Subtopics**:
- Matrix Operations - Part 1
- Matrix Operations - Part 2
- Matrix Operations - Part 3
- Matrix Operations - Part 4

**Learning Objectives**:
- Understand matrix operations
- Apply matrix operations concepts
- Implement matrix operations solutions

**Readings**:
- Chapter 2
- Research Paper 2

**Assignments Due**:
- Homework 1


### Week 3: Systems of Linear Equations

**Subtopics**:
- Systems of Linear Equations - Part 1
- Systems of Linear Equations - Part 2
- Systems of Linear Equations - Part 3
- Systems of Linear Equations - Part 4

**Learning Objectives**:
- Understand systems of linear equations
- Apply systems of linear equations concepts
- Implement systems of linear equations solutions

**Readings**:
- Chapter 3
- Research Paper 3


### Week 4: Linear Independence and Basis

**Subtopics**:
- Linear Independence and Basis - Part 1
- Linear Independence and Basis - Part 2
- Linear Independence and Basis - Part 3

**Learning Objectives**:
- Understand linear independence and basis
- Apply linear independence and basis concepts
- Implement linear independence and basis solutions

**Readings**:
- Chapter 4
- Research Paper 4

**Assignments Due**:
- Homework 2


### Week 5: Linear Transformations

**Subtopics**:
- Linear Transformations - Part 1
- Linear Transformations - Part 2

**Learning Objectives**:
- Understand linear transformations
- Apply linear transformations concepts
- Implement linear transformations solutions

**Readings**:
- Chapter 5
- Research Paper 5


### Week 6: Determinants

**Subtopics**:
- Determinants - Part 1
- Determinants - Part 2
- Determinants - Part 3

**Learning Objectives**:
- Understand determinants
- Apply determinants concepts
- Implement determinants solutions

**Readings**:
- Chapter 6
- Research Paper 6

**Assignments Due**:
- Homework 3


### Week 7: Eigenvalues and Eigenvectors

**Subtopics**:
- Eigenvalues and Eigenvectors - Part 1
- Eigenvalues and Eigenvectors - Part 2
- Eigenvalues and Eigenvectors - Part 3

**Learning Objectives**:
- Understand eigenvalues and eigenvectors
- Apply eigenvalues and eigenvectors concepts
- Implement eigenvalues and eigenvectors solutions

**Readings**:
- Chapter 7
- Research Paper 7

**Assignments Due**:
- Midterm Exam


### Week 8: Diagonalization

**Subtopics**:
- Diagonalization - Part 1
- Diagonalization - Part 2

**Learning Objectives**:
- Understand diagonalization
- Apply diagonalization concepts
- Implement diagonalization solutions

**Readings**:
- Chapter 8
- Research Paper 8

**Assignments Due**:
- Homework 4


### Week 9: Singular Value Decomposition

**Subtopics**:
- Singular Value Decomposition - Part 1
- Singular Value Decomposition - Part 2
- Singular Value Decomposition - Part 3
- Singular Value Decomposition - Part 4

**Learning Objectives**:
- Understand singular value decomposition
- Apply singular value decomposition concepts
- Implement singular value decomposition solutions

**Readings**:
- Chapter 9
- Research Paper 9


### Week 10: Principal Component Analysis

**Subtopics**:
- Principal Component Analysis - Part 1
- Principal Component Analysis - Part 2
- Principal Component Analysis - Part 3
- Principal Component Analysis - Part 4

**Learning Objectives**:
- Understand principal component analysis
- Apply principal component analysis concepts
- Implement principal component analysis solutions

**Readings**:
- Chapter 10
- Research Paper 10

**Assignments Due**:
- Homework 5
- Project Proposal


### Week 11: Matrix Factorization

**Subtopics**:
- Matrix Factorization - Part 1
- Matrix Factorization - Part 2
- Matrix Factorization - Part 3

**Learning Objectives**:
- Understand matrix factorization
- Apply matrix factorization concepts
- Implement matrix factorization solutions

**Readings**:
- Chapter 11
- Research Paper 11


### Week 12: Norms and Inner Products

**Subtopics**:
- Norms and Inner Products - Part 1
- Norms and Inner Products - Part 2
- Norms and Inner Products - Part 3

**Learning Objectives**:
- Understand norms and inner products
- Apply norms and inner products concepts
- Implement norms and inner products solutions

**Readings**:
- Chapter 12
- Research Paper 12

**Assignments Due**:
- Project Draft


### Week 13: Applications in ML

**Subtopics**:
- Applications in ML - Part 1
- Applications in ML - Part 2
- Applications in ML - Part 3

**Learning Objectives**:
- Understand applications in ml
- Apply applications in ml concepts
- Implement applications in ml solutions

**Readings**:
- Chapter 13
- Research Paper 13


### Week 14: Final Exam Review

**Subtopics**:
- Final Exam Review - Part 1
- Final Exam Review - Part 2

**Learning Objectives**:
- Understand final exam review
- Apply final exam review concepts
- Implement final exam review solutions

**Readings**:
- Chapter 14
- Research Paper 14

**Assignments Due**:
- Final Project


## Tags

`linear algebra`, `mathematics`, `machine learning`, `vectors`
