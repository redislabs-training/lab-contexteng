...
async def execute_react_tool(tool_name: str, tool_input: Dict[str, Any]) -> str:
    ...
    """
    try:
        if tool_name == "search_courses_hybrid":
            from .tools import search_courses_hybrid

            result = await search_courses_hybrid.ainvoke(tool_input)
            return result
        else:
            return f"Error: Unknown tool '{tool_name}'. Available tools: search_courses_hybrid"
