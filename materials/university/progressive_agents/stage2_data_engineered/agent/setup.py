"""
Setup and initialization for Stage 2 Data Engineered Agent.
...
def setup_agent(
    course_manager: Optional[CourseManager] = None, auto_load_courses: bool = True, verbose: bool = True,
) -> tuple:
    """
    Initialize the Stage 2 Baseline RAG agent.
    ...
    logger.info("🚀 Initializing Stage 2 Data Engineered Agent...")
    ...
    if auto_load_courses:
        ...
        try:
            ...
            # No loop yet: create one and run the coroutine synchronously
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)

        if loop.is_running():
            # We're in an environment like Jupyter with a running event loop
            try:
                # If nest_asyncio is available, use it so we can still block
                ...
                logger.info(f"✅ {courses_loaded} courses available (via nest_asyncio)")
            except ImportError:
                # Fallback: schedule in the background and warn the user
                logger.warning(
                    "Detected an already running event loop (likely Jupyter) and "
                    "nest_asyncio is not installed.\n"
                    "Courses will be loaded asynchronously in the background.\n"
                    "For deterministic behavior, call:\n"
                    "    await load_courses_if_needed(course_manager)\n"
                    "in your notebook after calling setup_agent()."
                )
                asyncio.ensure_future(load_courses_if_needed(course_manager))
        else:
            # Normal script / no running loop: safe to block
            courses_loaded = loop.run_until_complete(
                load_courses_if_needed(course_manager)
            )
    ...
    logger.info("✅ Stage 2 Data Engineered Agent initialized")
    logger.info("✨ This agent uses Section 2 data engineering techniques!")
